...
adb exec-out screencap -p > /tmp/loga3-shots/fetch-ui.png
cp /tmp/loga3-shots/fetch-ui.png .screenshots/fetch-ui.png 2>/dev/null || true
...
for i in $(seq 1 90); do
  ...
  adb exec-out screencap -p > /tmp/loga3-shots/fetch-progress.png
  python3 - <<'PY'
...
adb exec-out screencap -p > /tmp/loga3-shots/fetch-after.png
cp /tmp/loga3-shots/fetch-after.png .screenshots/fetch-after.png 2>/dev/null || true
python3 - <<'PY'
import re, pathlib
xml = pathlib.Path("/tmp/loga3-ui-fetch.xml").read_text(errors="ignore")
